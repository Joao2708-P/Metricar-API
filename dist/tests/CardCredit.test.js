...
// src/tests/CardCredit.test.ts
var import_bcrypt2 = __toESM(require("bcrypt"));
describe("CardCredit Model", () => {
  beforeEach(async () => {
    await prismaClient_default.creditCard.deleteMany();
  });
  describe("createCardCredit", () => {
    it("should create a new credit card", async () => {
      ...
      const card = await Card_credit_service_default.createCardCredit(cardData);
      expect(card).to.have.property("cardNumber", cardData.cardNumber);
      const isPasswordMatch = await import_bcrypt2.default.compare(cardData.password, card.password);
    ...
    it("should throw an error if card already exists", async () => {
      ...
      await Card_credit_service_default.createCardCredit(cardData);
      try {
        await Card_credit_service_default.createCardCredit(cardData);
      } catch (error) {
        expect(error).to.equal("Cart\xE3o j\xE1 cadastrado.");
      }
    });
  });
  describe("updateCard", () => {
    it("should update existing credit card", async () => {
      ...
      const updatedCard = await Card_credit_service_default.updateCard(cardData.cardNumber, updateData);
      expect(updatedCard).to.have.property("cardNumber", cardData.cardNumber);
      const isPasswordMatch = await import_bcrypt2.default.compare(updateData.password, updatedCard.password);
      expect(isPasswordMatch).to.be.true;
    });
    it("should throw an error if card is not found", async () => {
      ...
      const updateData = { password: "newpassword123" };
      try {
        await Card_credit_service_default.updateCard(cardNumber, updateData);
      } catch (error) {
        expect(error).to.equal("Cart\xE3o n\xE3o encontrado.");
      }
    });
  });
});
