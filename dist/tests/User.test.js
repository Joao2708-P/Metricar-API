...
describe("UserService Model", () => {
  beforeEach(async () => {
    await prismaClient_default.user.deleteMany();
  });
  describe("createUser", () => {
    it("should create a new user", async () => {
      ...
      const user = await CreateUser_Service_default.createUser(userData);
      expect(user).to.have.property("name", userData.name);
      expect(user).to.have.property("email", userData.email);
      const isPasswordMatch = await import_bcrypt2.default.compare(userData.password, user.password);
    ...
    it("should throw an error if email already exists", async () => {
      ...
      await CreateUser_Service_default.createUser(userData);
      try {
        await CreateUser_Service_default.createUser(userData);
      } catch (error) {
        expect(error).to.equal("Email de usu\xE1rio j\xE1 cadastrado.");
      }
    });
    it("should throw an error for invalid email format", async () => {
      ...
      try {
        ...
        expect(error).to.include("Email inv\xE1lido!");
      }
    });
    it("should throw an error for short password", async () => {
      ...
      try {
        ...
        expect(error).to.include("Senha precisa conter no m\xEDnimo 8 caracteres.");
      }
    });
  });
});
